...
parser.add_argument('--tracker_type', default='SiamMask', type=str, help='Tracker algorithm to be used')
...
class LabelTracker():
    ...
    def __init__(self, tracker_type, init_frame, next_frame_path_list):
        tracker_types = ['CSRT', 'KCF','MOSSE', 'MIL', 'BOOSTING', 'MEDIANFLOW', 'TLD', 'GOTURN', 'SiamMask']
    ...
    def call_tracker_constructor(self, tracker_type):
        ...
        # --
            ...
            if tracker_type == 'CSRT':
                ...
                tracker = Custom(anchors=self.cfg['anchors'])
                tracker = load_pretrain(tracker, "../siamMask/pretrained/SiamMask_VOT.pth", use_cuda=self.use_cuda)
    ...
    def start_tracker(self, json_file_data, json_file_path, img_path, obj, color, annotation_formats):
        ...
        def on_press(key):
            ...
            if key == keyboard.Key.space or k == 'p':
                done.add(True)
                return False
        ...
        if self.tracker_type == 'SiamMask':
            target_pos = np.array([(xmin + xmax) / 2, (ymin + ymax) / 2])
            target_sz = np.array([xmax - xmin, ymax - ymin])

            state = siamese_init(self.init_frame, target_pos, target_sz, tracker, self.cfg['hp'], use_cuda=self.use_cuda)  # init tracker
        else:
        ...
        for frame_path in self.next_frame_path_list:
            ...
            if self.tracker_type == 'SiamMask':
                state = siamese_track(state, next_image.copy(), mask_enable=True, refine_enable=True, use_cuda=self.use_cuda)  # track
                target_pos, target_sz = state['target_pos'], state['target_sz']
                bbox = np.concatenate([target_pos - target_sz/2, target_sz])
                success = True
            else:
        ...
        # show edges key listener
            ...
            if is_from_video:
                ...
                if len(object_list) > 0:
                    ...
                    # initial frame
                    init_frame = img.copy()
                    label_tracker = LabelTracker(args.tracker_type, init_frame, next_frame_path_list)
